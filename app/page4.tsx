...
function Insights({
  ...
  async function loadHistoricalData() {
    ...
    try {
      ...
      // with no guaranteed ordering unless we ask for one. With dense
      // sample data, a single .select() here was silently truncating at
      // 1000 rows in whatever arbitrary order Postgres returned them —
      // which is why some week buckets looked empty while others didn't.
      // Paginate through everything in the range instead, ordered by
      ...
      while (true) {
        ...
        console.log(
          `Fetched page: rows ${from}-${from + PAGE_SIZE - 1}, got ${
            (page || []).length
