...
function Insights({
  ...
  // "Revenue Drivers" — a single curated section (instead of separate
  // always-visible charts for hour/source/category) that lets an owner
  // toggle between three lenses on what's actually driving revenue.
  // Uses a fixed 30-day rolling window rather than just today, since a
  // single day's hour/source/category split is too sparse to draw a
  // real pattern from — staffing and channel decisions should be based
  // on a recent trend, not one day's noise.
  const REVENUE_DRIVER_WINDOW_DAYS = 30;

  const [
    revenueDriverMetric,
    setRevenueDriverMetric,
  ] = useState<
    "hour" | "source" | "category"
  >("hour");

  function revenueDriverMetricLabel(
    m: "hour" | "source" | "category"
  ) {
    if (m === "hour")
      return "By Hour";
    if (m === "source")
  ...
  const revenueDriverOrders = useMemo(
    () => {
      const cutoff = new Date(today);
      cutoff.setDate(
        cutoff.getDate() -
          REVENUE_DRIVER_WINDOW_DAYS
      );
  ...
  const revenueDriverData = useMemo(
    (): RevenueDriverDatum[] => {
      if (
        revenueDriverMetric === "hour"
      ) {
        const buckets: RevenueDriverDatum[] =
          new Array(24)
            .fill(null)
            .map((_, h) => ({
              key: String(h),
      ...
      if (
        ...
        revenueDriverOrders.forEach(
          (o) => {
            ...
            map[o.source].revenue +=
              o.total;
            map[o.source].orders += 1;
          }
        );
  ...
  const revenueDriverTotal =
    revenueDriverData.reduce(
      (s, d) => s + d.revenue,
      0
    );

  const revenueDriverTop = [
    ...revenueDriverData,
  ].sort(
    (a, b) => b.revenue - a.revenue
  )[0];

  const revenueDriverTopSharePct =
    revenueDriverTop &&
    revenueDriverTotal > 0
      ? (revenueDriverTop.revenue /
          revenueDriverTotal) *
        100
      : 0;

  // One-sentence, data-grounded takeaway per lens — no invented figures
  // (e.g. no assumed commission rates for delivery channels), just what
  // can actually be derived from the order data itself.
  function buildRevenueDriverInsight() {
    if (
      !revenueDriverTop ||
      revenueDriverTotal === 0
    ) {
      return "Not enough order history in the last 30 days to surface a pattern yet.";
    }

    const sharePct =
      revenueDriverTopSharePct.toFixed(
        0
      );
    ...
    if (
      ...
      return `Your peak revenue window is ${revenueDriverTop.label}, bringing in ${moneyCompact(revenueDriverTop.revenue)} (${sharePct}% of the last 30 days' revenue) — worth planning staffing around this window.`;
    }

    if (
      revenueDriverMetric === "source"
    ) {
      const aovs =
        revenueDriverData.map(
          (d) => ({
            label: d.label,
            aov:
              d.secondary > 0
      ...
      const topAov = aovs.find(
        (a) =>
          a.label ===
          revenueDriverTop.label
      );
      const highestAov = [...aovs].sort(
        (a, b) => b.aov - a.aov
      )[0];
      ...
      const aovNote =
        ...
        topAov.aov <
          highestAov.aov * 0.9
          ? ` Its average order value (${moneyCompact(topAov.aov)}) is lower than ${highestAov.label}'s (${moneyCompact(highestAov.aov)}) — worth keeping an eye on margins.`
          : "";

      return `${revenueDriverTop.label} is your top channel, driving ${sharePct}% of revenue over the last 30 days.${aovNote}`;
    }

    return `${revenueDriverTop.label} is your best-selling category, contributing ${sharePct}% of revenue over the last 30 days.`;
  }
  ...
  return (
    <>
      ...
      {/* Revenue Drivers — one curated section covering hour, source,
          and category, toggled with pills rather than shown as three
          separate always-visible charts. Built on a rolling 30-day
          window (not just today) so the pattern is stable enough to
          actually act on, with a plain-language takeaway under the
          chart that's derived directly from the data, not invented. */}
      <div className="card">
        ...
        <div className="chart">
          ...
          >
            <BarChart
              data={revenueDriverData}
            >
              <defs>
                <linearGradient
                  id="revenueDriverHighlightGradient"
                  x1="0"
                  y1="0"
                  x2="0"
                  y2="1"
                ...
                labelFormatter={(
                  ...
                  const secondary =
                    payload?.[0]
                      ?.payload;
                  return secondary
                    ? `${label} — ${secondary.secondary} ${secondary.secondaryLabel}`
                    : label;
                }}
              />
              <Bar
                  ...
                  (entry) => (
                    <Cell
                      ...
                      fill={
                        revenueDriverTop &&
                        entry.key ===
                          revenueDriverTop.key
                          ? "url(#revenueDriverHighlightGradient)"
                          : "url(#revenueDriverMutedGradient)"
                      }
                    />
                  )
                )}
        ...
        <div className="ai-insight-banner">
          <span className="ai-insight-tag">
            Insight
          </span>
          <span>
            {buildRevenueDriverInsight()}
          </span>
        </div>
